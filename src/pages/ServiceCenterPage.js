...
// ── Geocode using FREE Nominatim (no Google Geocoding API needed) ─────────────
async function geocodeCity(city) {
  try {
    ...
    throw new Error('Could not locate city. Please try a different name.');
  }
}
...
function GoogleMap({ centers, center }) {
  ...
  useEffect(() => {
    ...
    centers.forEach((c, i) => {
      ...
      const info = new window.google.maps.InfoWindow({
        content: `<div style="color:#000;font-size:13px;padding:4px 2px">
          <strong>${c.name}</strong><br/>
          ${c.address}<br/>
          ${c.rating !== 'N/A' ? `⭐ ${c.rating}` : ''}
          ${c.isOpen === true ? ' · <span style="color:green">Open</span>' : c.isOpen === false ? ' · <span style="color:red">Closed</span>' : ''}
        </div>`,
      });
  ...
  return (
    <div ref={mapRef} style={{ width:'100%', height:260, borderRadius:14, border:'1px solid var(--border)', marginBottom:16, overflow:'hidden' }} />
  );
}
...
export default function ServiceCenterPage({ navigate }) {
  ...
  useEffect(() => {
    loadGoogleMaps().then(() => setMapReady(true)).catch(() => {});
  }, []);
  ...
  const search = async () => {
    ...
    try {
      ...
      // Search Google Places
      const query = brand
        ? `${brand} authorized service center ${city} India`
        : `two wheeler bike scooter service center ${city} India`;

      const places = await searchNearby(query, coords.lat, coords.lng);

      if (places.length === 0) {
        setError(`No service centers found near "${city}". Try a more specific area name.`);
        setLoading(false);
        return;
      }
  ...
  const useGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setError('GPS not available. Please enter your city manually.'); setLocating(false); }
    );
  };
      ...
      <div style={s.container}>
        <div style={s.card}>
          ...
          <div style={s.inputRow}>
            <input
              style={s.input}
              placeholder="e.g. Nashik, Andheri Mumbai, Koramangala"
              value={city}
              onChange={e => setCity(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && search()}
            />
            <button style={s.gpsBtn} onClick={useGPS} title="Use my location">
              {locating ? '⌛' : '📍'}
            </button>
          </div>
          ...
          <button
            style={{ ...s.btn, opacity: loading || !city.trim() ? 0.6 : 1 }}
            onClick={search}
        ...
        {loading && (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ ...s.centerCard, height:100, animation:'pulse 1.2s ease infinite', animationDelay:`${i*0.15}s` }} />
            ))}
          </div>
        )}
            ...
            {centers.map((c, i) => (
              <div key={i} style={s.centerCard}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:6 }}>
                  {c.distance && (
                    <span style={{ fontSize:11, color:'var(--accent)', background:'rgba(0,212,255,0.08)', border:'1px solid rgba(0,212,255,0.15)', borderRadius:6, padding:'2px 8px' }}>
                      📏 {c.distance} km
                    </span>
                  )}
                ...
                <div style={s.actionRow}>
                  ...
                  <div style={s.mapBtn} onClick={() => window.open(c.mapsUrl, '_blank')}>
                    🗺️ Open in Maps
                  </div>
                </div>
              </div>

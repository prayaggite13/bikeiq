...
// Search Places API for dealers near a location
  ...
  useEffect(() => {
    ...
    dealers.forEach((d, i) => {
      ...
      const info = new window.google.maps.InfoWindow({
        content: `<div style="color:#000;font-size:13px;padding:4px"><strong>${d.name}</strong><br/>${d.address}<br/>⭐ ${d.rating}</div>`,
      });
...
export default function DealerLocatorPage({ navigate }) {
  ...
  const search = async () => {
    ...
    try {
      ...
      const query = brand
        ? `${brand} bike dealer ${city}`
        : `two wheeler bike dealer ${city}`;

      const places = await searchNearby(query, coords.lat, coords.lng);
      if (places.length === 0) { setError('No dealers found. Try a different city or brand.'); setLoading(false); return; }

      // Take top 6, fetch details in parallel
      ...
      <div style={s.container}>
        <div style={s.card}>
          ...
          <div style={s.inputRow}>
            <input style={s.input} placeholder="e.g. Mumbai, Koramangala Bangalore" value={city}
          ...
          <button style={{ ...s.btn, opacity: loading || !city.trim() ? 0.6 : 1 }}
            ...
            {loading ? '🔍 Searching Google Maps...' : '🔍 Find Real Dealers'}
          </button>
        </div>
        ...
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ ...s.dealerCard, height: 100, animation: 'pulse 1.2s ease infinite', animationDelay: `${i*0.15}s` }} />
            ))}
          </div>
        )}
        ...
        {!loading && dealers.length > 0 && (
          <>
            <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 10 }}>
              📍 {dealers.length} dealers found near {city}
            </div>
